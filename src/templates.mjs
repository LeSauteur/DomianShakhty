...
function constructionPrice(item) {
  ...
  if (item.priceStatus === "partner-outdated") return { value: `от ${formatPrice(item.price)}`, note: item.priceDate ? `Ориентир из презентации за ${item.priceDate}; актуальную смету нужно подтвердить` : "Ориентир из презентации без указанной даты; актуальную смету нужно подтвердить" };
  return { value: `от ${formatPrice(item.price)}`, note: `Стоимость комплектации «${item.pricePackage || "уточняется"}» по данным ${item.priceDate || "источника"}; нужна актуализация` };
}
...
function constructionMonthlyPayment(item) {
  ...
  return `от ${formatPrice(item.monthlyPayment)}/мес. по расчёту презентации${terms ? ` (${terms})` : ""}`;
}
...
export function renderConstructionProject(ctx, item) {
  ...
  const facts = [["Вариант", item.code], ["Подборка", item.builder], ["Тип", item.projectType === "individual" ? "Индивидуальный" : "Типовой"], ["Площадь", `${String(item.area).replace(".", ",")} м²`], ["Этажность", item.floors], ["Спальни", item.bedrooms], ["Санузлы", item.bathrooms], ["Материалы", item.material], ["Комплектация", item.pricePackage], ["Расчёт платежа", payment]].filter(([, value]) => value !== null && value !== undefined && value !== "");
  ...
  const gallery = galleryMedia.length ? `<section class="section catalog-gallery"><div class="container">${sectionHeading(galleryHeading)}<div class="catalog-gallery__grid">${galleryMedia.map((media) => {
    const image = catalogPicture(ctx, media, { sizes: "(max-width: 820px) calc(100vw - 32px), 48vw" });
...
export function renderPerson(ctx, person = ctx.team.find((item) => item.id === "maria-voronina")) {
  ...
  const path = `team/${person.slug || person.id}.html`;
  const page = { path, pageType: "person", person, title: `${person.name} — кадастровый инженер и агент | Домиан Шахты`, description: `${person.name}: кадастровый инженер и агент по продаже недвижимости в Шахтах и соседних территориях. Направления работы и прямые контакты.`, eyebrow: "Кадастровый инженер · агент", h1: person.name, lead: "Помощь с оценкой и подготовкой объекта, документами, проверкой покупателей и сопровождением продажи недвижимости.", primaryCta: { label: "Позвонить Ольге", href: person.phoneHref, event: "phone_click" }, secondaryCta: { label: "Вся команда", href: "team/index.html" }, heroFacts: ["квартиры · участки · дома", "строительство", "Шахты и соседние территории"], heroImage: "client-meeting" };
  const profile = `<section class="section"><div class="container profile-layout">${personPortrait(ctx, person, "large")}<div><p class="eyebrow">Прямой контакт</p><h2>${esc(person.role)}</h2><p>Ольга работает с квартирами, земельными участками, домовладениями и вопросами строительства. География подбирается под задачу клиента без публикации точных адресов объектов.</p><dl class="profile-facts"><div><dt>Телефон</dt><dd><a href="${esc(person.phoneHref)}" data-analytics="phone_click">${esc(person.phone)}</a></dd></div><div><dt>MAX</dt><dd>${esc(person.maxPhone)} · поиск по номеру</dd></div><div><dt>Email</dt><dd><a href="mailto:${esc(person.email)}" data-analytics="email_click">${esc(person.email)}</a></dd></div><div><dt>Офис</dt><dd>${esc(person.office)}</dd></div></dl></div></div></section>`;
  const directions = cardsSection(ctx, { kicker: "Основные направления", title: "С какими объектами работает Ольга", intro: "Состав сопровождения определяется после знакомства с объектом и документами.", items: person.directions.map((title, index) => ({ index: String(index + 1).padStart(2, "0"), title, text: ["Подбор и продажа квартир с проверкой исходных данных.", "Проверка параметров земли и подготовка к сделке.", "Продажа домов с участками и сопоставление документов.", "Подбор сценария строительства и проверка предложения." ][index] })) });
  const skills = `<section class="section section--stone"><div class="container">${sectionHeading({ kicker: "Навыки", title: "Подготовка и сопровождение продажи", intro: "Работа выстраивается вокруг проверяемых данных, понятной презентации и согласованных условий сделки." })}<ol class="agent-skill-list">${person.skills.map((skill, index) => `<li><span>${String(index + 1).padStart(2, "0")}</span><strong>${esc(skill)}</strong></li>`).join("")}</ol></div></section>`;
  const areas = `<section class="section"><div class="container agent-area-layout"><div><p class="eyebrow">География работы</p><h2>Шахты и соседние территории</h2><p>Точный адрес объекта не публикуется без отдельного согласования.</p></div><ul>${person.serviceAreas.map((area) => `<li>${esc(area)}</li>`).join("")}</ul></div></section>`;
  return layout(ctx, page, `${hero(ctx, page)}${profile}${directions}${skills}${areas}${leadForm(ctx, { type: "service", title: "Передать задачу Ольге", text: "Укажите тип недвижимости, территорию и желаемый результат. Не отправляйте чувствительные документы через форму." })}`, { active: "olga-chernenko", breadcrumbs: [{ label: "Главная", href: "" }, { label: "Команда", href: "team/index.html" }, { label: person.shortName || person.name, href: page.path }] });
}
